...
ifCleanToken <- function(cur_token){
  ...
  rule_list <- c("str_count(cur_token, pattern = '[A-Za-z0-9]') <= 0.5*nchar(cur_token)", # If the number of punctuation characters in a string is greater than the number of alphanumeric characters, it is garbage
                 ...
                 "grepl('^[A-Za-z]+$', cur_token, perl = T) & ((8*str_count(cur_token,char_class('aeiouAEIOU')) < str_count(cur_token,pattern= '[b-df-hj-np-tv-zB-DF-HJ-NP-TV-Z]')) | (8*str_count(cur_token,pattern= '[b-df-hj-np-tv-zB-DF-HJ-NP-TV-Z]') < str_count(cur_token,char_class('aeiouAEIOU'))))", #If all the characters in a string are alphabetic, and if the number of consonants in the string is greater than 8 times the number of vowels in the string, or vice-versa, it is garbage
